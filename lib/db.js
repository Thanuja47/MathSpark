...
export const db = {
  students: {
    ...
    /** Find student by ID including gradeAccess */
    ...
    /** Get allowed grade IDs for a student ID */
    ...
    /** Update student grade access (sets exact list of grade IDs) */
    setApprovedGrades: async (studentId, gradeIds) => {
      const prisma = getPrisma();
      // Transaction to wipe and re-insert granted grades
      return prisma.$transaction([
        prisma.studentGradeAccess.deleteMany({ where: { studentId } }),
        prisma.studentGradeAccess.createMany({
          data: gradeIds.map(gId => ({
            studentId,
            gradeId: Number(gId)
          }))
        })
  ...
  lessons: {
    all: async () => {
      return getPrisma().lesson.findMany({ orderBy: { createdAt: 'desc' } });
    },
    findById: async (id) => {
      return getPrisma().lesson.findUnique({ where: { id } });
    },
    create: async (data) => {
      return getPrisma().lesson.create({
        data: {
          title:       data.title,
    ...
    update: async (id, data) => {
      return getPrisma().lesson.update({
        where: { id },
        data: {
          ...(data.title       !== undefined && { title: data.title }),
          ...(data.gradeId     !== undefined && { gradeId: Number(data.gradeId) }),
          ...(data.description !== undefined && { description: data.description || null }),
    ...
    delete: async (id) => {
      return getPrisma().lesson.delete({ where: { id } });
    }
  }
};
